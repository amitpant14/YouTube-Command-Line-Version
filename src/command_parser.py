...
class CommandParser:
    ...
    def process_command(self, command: str, matching_videos: List[Video]):
        command = int(command)
        if command<1 or command > len(matching_videos)+1:
            raise CommandException()
        else:
            self._player.play_video(matching_videos[command-1].video_id)
